...
import { useEcommerceSellers } from "./use-sellers";

const SellerRow = ({ seller }: { seller: IEcommerceSeller }) => {
    const { showDeleteSellerConfirmation, handleSelectSeller, selectedSellerIds } = useEcommerceSellers();
    const { id, name, date, image, shopName, email, sales, gender, earning, verified, mobileNumber } = seller;

    return (
        <>
            <TableRow className="cursor-pointer hover:bg-base-200/40" onClick={() => handleSelectSeller(id)}>
                <Checkbox
                    size={"xs"}
                    checked={selectedSellerIds.includes(id)}
                ...
                <div className="inline-flex w-fit">
                    <Link
                        href={routes.apps.ecommerce.sellers.edit(id)}
                    ...
                    <Button
                        ...
                        onClick={(event) => {
                            event.stopPropagation();
                            showDeleteSellerConfirmation(id);
                        }}>
...
const EcommerceSellerTable = () => {
    ...
    return (
        <>
            <Card className="bg-base-100">
                <CardBody className={"p-0"}>
                    <div className="flex items-center justify-between px-5 pt-5">
                        ...
                        <div className="inline-flex items-center gap-3">
                            <Link href={routes.apps.ecommerce.sellers.create} aria-label={"Create seller"}>
                    ...
                    <div className="overflow-auto">
                        <Table className="mt-2 rounded-box">
                            <TableHead>
                                <Checkbox
                                    size={"xs"}
                                    indeterminate={sellersSelectionStatus === "intermediate"}
                                    checked={sellersSelectionStatus === "all"}
                                    onChange={() => handleSelectAllSeller()}
                            ...
                            <TableBody>
                                {sellers.slice(0, 11).map((seller, index) => (
            ...
            <Modal ref={sellerDeleteConfirmationRef} backdrop>
                <form method="dialog">
                    <Button
                        size="sm"
                        color="ghost"
                ...
                <ModalBody>
                    You are about to delete <b>{sellerToBeDelete?.name}</b>. Would you like to proceed further ?
                </ModalBody>
                <ModalActions>
                    <form method="dialog">
                    ...
                    <form method="dialog">
                        <Button color="primary" size="sm" onClick={() => handleDeleteSeller()}>
                            Yes
                        </Button>
                    </form>
                </ModalActions>
